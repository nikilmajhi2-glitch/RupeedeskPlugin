...
public class AutoSmsWorker extends Worker {
    ...
    private int subscriptionId; // <-- ADDED
    ...
    public Result doWork() {
        ...
        subscriptionId = prefs.getInt("subscriptionId", -1); // <-- ADDED: Get the saved ID
                ...
                .addOnSuccessListener(snapshot -> {
                    ...
                    for (DocumentSnapshot doc : snapshot.getDocuments()) {
                        ...
                        if (tasksToLease > 0 && ("pending".equals(status) || "failed".equals(status))) {
                            ...
                            // We pass subscriptionId to the next method
                            leaseAndProcessTask(doc, subscriptionId); // <-- MODIFIED
                            tasksToLease--;
                        }
                    }
    ...
    private void leaseAndProcessTask(DocumentSnapshot doc, int subId) { // <-- MODIFIED: Add subId
        String id = doc.getId();
        ...
        db.runTransaction((Transaction.Function<DocumentSnapshot>) transaction -> {
            ...
            if (snapshot != null) {
                ...
                sendSms(snapshot, userId, subId); // <-- MODIFIED: PASS subId
            }
        }).addOnFailureListener(e -> {
    ...
    // MODIFIED: Change the signature to accept the subId
